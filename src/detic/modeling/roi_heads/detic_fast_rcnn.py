...
__all__ = ["fast_rcnn_inference_with_box_features", "DeticFastRCNNOutputLayers"]


def fast_rcnn_inference_with_box_features(
    ...
    Args:
        ...
        box_features (list[Tensor]): A list of Tensors of box features (the input of the final prediction layer)
            for each image i.
    ...
    for scores_per_image, boxes_per_image, box_features_per_image, image_shape in  \
            zip(scores, boxes, box_features, image_shapes):
        result, filtered_indices = fast_rcnn_inference_single_image_with_box_features(
            boxes_per_image, scores_per_image, box_features_per_image, image_shape,
            score_thresh, nms_thresh, topk_per_image
        )
        results.append(result)
        filtered_indices_list.append(filtered_indices)
    return results, filtered_indices_list


def fast_rcnn_inference_single_image_with_box_features(
        boxes,
        scores,
        box_features,
    ...
    box_features = box_features.view(-1, num_bbox_reg_classes, 512)  # R x C x 512
    ...
    if num_bbox_reg_classes == 1:
        ...
        box_features = box_features[filter_inds[:, 0], 0]
    else:
    ...
    result.pred_box_features = box_features
...
class DeticFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def forward(self, x, classifier_info=(None, None, None)):
        ...
        if classifier_info[0] is not None:
            cls_scores, features = self.cls_score(x, classifier=classifier_info[0])
            scores.append(cls_scores)
        else:
            cls_scores, features = self.cls_score(x)
            scores.append(cls_scores)
        ...
        scores = torch.cat(scores, dim=1)  # B x C' or B x N or B x (C'+N), e.g. 256 x 1204 for lvis_v1

        proposal_deltas = self.bbox_pred(x)
        if self.with_softmax_prop:
            prop_score = self.prop_score(x)
            return scores, proposal_deltas, prop_score, features
        else:
            return scores, proposal_deltas, features
