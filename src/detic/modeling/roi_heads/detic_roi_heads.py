...
from .detic_fast_rcnn import DeticFastRCNNOutputLayers, fast_rcnn_inference_with_box_features
    ...
    def _forward_box(self, features, proposals, targets=None,
        ...
        for k in range(self.num_cascade_stages):
            if k > 0:
                ...
                proposals = self._create_proposals_from_boxes(prev_pred_boxes, image_sizes, logits=logits)
        ...
        if self.training:
            ...
            last_head_scores = predictions[0]
            last_head_proposal_deltas = predictions[1]
            last_head_box_features = predictions[-1]

            num_prop_per_image = [len(p) for p in proposals]
            last_head_box_features = last_head_box_features.split(num_prop_per_image)

            boxes = predictor.predict_boxes((last_head_scores, last_head_proposal_deltas), proposals)
            pred_instances, _ = fast_rcnn_inference_with_box_features(
                boxes,
                scores,
                last_head_box_features,
                image_sizes,
